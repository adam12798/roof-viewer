...
1. **Orientation tuning complete for now.** Inlier refit (±15cm, two-pass) + polygon erosion (0.5m) are the final settings. ±10cm rejected (§8.9), 1.0m rejected (§8.11). Further improvement requires a fundamentally different approach (RANSAC, weighted lstsq, or DSM resolution upgrade). Current stats: >40° faces reduced by 29%, >55° by 36% vs no-erosion baseline.
