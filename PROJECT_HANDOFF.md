...
1. **Orientation tuning: tighter inlier threshold or polygon erosion.** Live validation (§8.8) shows the refit produces consistent 1–4° corrections but smaller than predicted. Root cause: ±15cm inlier threshold includes mildly contaminated edge points. Options: tighter threshold (±10cm), polygon erosion before DSM sampling, or weighted lstsq. Not urgent — current refit is safe and directionally correct.
