...
1. **Implement two-pass inlier refit in orientation module.** Investigation complete (see `ML_AUTO_BUILD_TRIAGE_STATUS.md` §8). Root cause: single-pass lstsq in `_fit_plane()` is contaminated by edge/wall pixels. Flagged faces (RMSE>0.30 or inlier<0.60) have +15–17° tilt bias vs unflagged. Fix: when inlier_ratio < 0.60 and n_inliers ≥ 12, refit lstsq on inlier samples only. ~15 lines in `ml_engine/core/stages/orientation.py:403-450`. Expected: 40–55° faces drop to 25–38° range.
